...
- Error handling in Rocket turned out to be more of a pain that I expected. The errors come in as integers, then I need to convert them to C strings, then to responses of the correct type, but also the functions all had to be strongly typed properly to handle the errors. It probably wouldn't have taken more than an hour to get it working and have every failure case that currently panics return a proper response that propagates up the functions and gets returned to the user as a properly formatted response, but I preferred to focus on meeting spec.
